...
class Safety extends Backend{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            if($adminId==1){
                $map='group_id=14 or group_id=15 or group_id=16 or group_id=17';
            }else{
                if($admin['group_id']==14){
                    //如果是站长
                    $map='group_id=15 or group_id=16 or group_id=17';
                }elseif ($admin['group_id']==15){
                    //如果是副站长
                    $map='group_id=16 or group_id=17';
                }
            }
        ...
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                try{
                    if($level==0){
                        //站长 先判断是否已经有站长
                        if(db('auth_group_access')->where(['group_id'=>14])->find()){
                            $this->error("已经有一个站长了");
                        }
                        $data['group_id']=14;
                    }elseif ($level==1){
                        //副站 先判断是否已经有两个副站了
                        if(db('auth_group_access')->where(['group_id'=>15])->count()==2){
                            $this->error("已经有两个副站了");
                        }
                        $data['group_id']=15;
                    }elseif ($level==2){
