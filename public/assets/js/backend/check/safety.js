        ...
        index: function () {
            ...
            $(document).ready(function () {
                ...
                buttons.push({
                    ...
                    url      : 'check/safety/notice?ret='+identy,
                    visible  : function (row) {
                        //返回true时按钮显示,返回false隐藏
                        if(identy==0){
                            //安监员
                            if(row.supervisor_progress==0)
                                return true;
                        }else if(identy==2){
                            //副站
                            if(row.supervisor_progress==1)
                                return true;
                        }else if(identy==1){
                             //站长
                            if(row.supervisor_progress==2)
                                return true;
                        }
                        return false;
                    }
            ...
            table.bootstrapTable({
                ...
                columns   : [
                    [
                        ...
                        {field: 'supervisor_progress', title: '安监进度', formatter:Controller.api.formatter.progress,searchList: {'0':'未处理','1': '已通知副站','2': '已通知主站','3': '主站同意'}},
